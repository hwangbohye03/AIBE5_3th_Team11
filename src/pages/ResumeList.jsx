    ...
    <div className="min-h-screen bg-[#FDFBF7] flex flex-col">
      ...
      <div className="bg-white border-b border-[#F3E8D0]">
        <div className="max-w-3xl mx-auto px-4 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-[#5D4037]">내 이력서</h1>
            <p className="text-sm text-gray-500 mt-1">작성한 이력서를 관리하고 채용공고에 지원해보세요.</p>
          </div>
          <Link
            to="/resumes/new"
      ...
      {/* 이력서 목록 */}
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4 flex-1 w-full">
        {resumes.length === 0 ? (
          <div className="text-center py-20 text-gray-400">
            <div className="text-5xl mb-4">📄</div>
            <p className="text-lg font-medium text-gray-600">아직 작성된 이력서가 없습니다.</p>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h2 className="font-bold text-[#5D4037] text-base truncate">{resume.title}</h2>
                    {resume.isDefault && (
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
            <h3 className="font-bold text-[#5D4037] text-lg mb-2">이력서를 삭제할까요?</h3>
